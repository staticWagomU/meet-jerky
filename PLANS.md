...
- [x] `utils/google-auth.ts` に OAuth 認証ヘルパーを作成
  ...
  - `revokeToken()`: ログアウト処理
  - `getAuthToken()`: 認証状態チェック（非対話的トークン取得）
- [x] `utils/google-sheets.ts` に Sheets API クライアントを作成
...
- [x] Options Page に Google 連携セクションを実装
  - [Google アカウントでログイン] ボタン
  - 認証状態表示（接続済み / 未接続）
  - [連携解除] ボタン
- [x] Popup の詳細画面に [Sheets] エクスポートボタンを追加
- [x] エクスポート実行時のプログレス表示（作成中... → 完了！リンクを開く）
- [x] エクスポート完了後にスプレッドシートのURLを表示（クリックで開く）
- [x] 未認証状態で Sheets ボタン押下時に Options Page への誘導
- [x] エラーハンドリング（認証失敗、API quota、ネットワークエラー）
