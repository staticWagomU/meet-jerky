...
- [x] `utils/__tests__/settings.test.ts` を作成（RED）
...
- [x] `entrypoints/background.ts` の `MAX_STORED_SESSIONS` 定数（L80）を設定値参照に変更
...
- [x] `chrome.storage.onChanged` リスナーを追加
  - `user-settings` キーの変更を監視
  - 設定変更時に即座にクリーンアップを実行（新しい制限値に基づく）
- [x] Service Worker 起動時にもクリーンアップを1回実行
...
### 4a: HTML テンプレート
- [x] `entrypoints/options/index.html` を作成
  - `popup/index.html` をベースに `#app` コンテナを配置

### 4b: メインロジック
- [x] `entrypoints/options/main.ts` を作成
...
### 4c: スタイル
- [x] `entrypoints/options/style.css` を作成
...
- [x] `entrypoints/popup/main.ts` の `renderSessionList()` ヘッダー部分に ⚙ アイコンリンクを追加
  - クリック時に `chrome.runtime.openOptionsPage()` を呼び出し
- [x] `entrypoints/popup/style.css` に ⚙ リンクのスタイルを追加

---

## 検証チェックリスト

- [x] `bun run test` — 全テスト通過（既存 + 新規 settings テスト）
- [x] `bun run lint` — Biome リント通過
- [x] `bun run build` — ビルド成功、Options Page が manifest に自動追加されていること
