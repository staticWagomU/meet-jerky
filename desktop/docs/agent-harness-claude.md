...
`docs/agent-harness.md` の Codex 版と同じ運用思想を、Claude Code (`claude` CLI) で動かすためのハーネス。tmux 上の Claude Code セッションを起動、監視、指示、再起動、引き継ぎできるようにし、自律改善と小さな新機能追加を安全・継続的に回す。

ハーネスはメインエージェントの判断を補助するためのものであり、判断責任を完全にスクリプトへ丸投げしない。
新機能追加の採否、範囲、優先順位、検証結果の解釈、コミット可否はメインエージェントが判断する。watchdog や補助スクリプトは判断・実装・差分修正・コミット判断を行わない。
...
- AGENTS.md と `docs/product-concept.md` に沿った改善・新機能追加候補の選定
- 作業分解
- tmux セッション起動
- pane 出力監視
- 作業指示
...
- コード変更禁止
- 調査、設計、リスク整理、改善提案、新機能候補の整理のみ行う
- 実装可能で価値が高い改善候補または新機能追加候補を報告する
- 新機能候補は、最小の検証可能スライス、ユーザー価値、非目標、権限・課金・プライバシー上の制約を明記する
...
- 他エージェントの変更を戻さない
- 新機能追加を担当する場合も、指定された最小スライスだけを実装し、大きな構想へ勝手に広げない
- 実装後に妥当な検証を実行する
...
- 課金・認証変更・ステルス録音・権限透明性の低下を伴う新機能追加
...
4. pane 出力 / 出力ファイルを確認し、改善候補または新機能追加候補を小さな作業単位に分解する。
