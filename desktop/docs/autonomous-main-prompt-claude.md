...
- 後継用プロンプトファイルには、目的、現在の git 状態、直近コミット、進行中セッション、未完了タスク、検証制約、旧メインが終了すべきことを具体的に書く。
- `scripts/claude-agent-handoff-main.sh mjc-main-YYYYMMDD-N /path/to/prompt.txt` で後継を起動する。
- 後継起動後、旧メインは `scripts/agent-adopt-main.sh mjc-main-YYYYMMDD-N mjc-main` で後継を canonical な `mjc-main` 名へ切り替え、作業を増殖させず終了する。
